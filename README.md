...
Таблица сообщений портала «Наш Город» в работе — на уровне отдельных заявок, с автоматическим трекингом статуса между выгрузками (не путать с «Дашборд ЮВАО», который агрегирует просрочки ММ по районам).
...
| GET    | `/api/ng_overdue`                 | —             | Дашборд НГ — сообщения в работе (NG_prosrok) |
