...
import { ArrowLeft, FileSpreadsheet, Clock, Info, MessageSquare } from 'lucide-react';
import PDFViewer from './PDFViewer';
import { useReportFiles } from '../hooks/useReportFiles';

interface OurCityReportProps {
  folderName: string;
}

const OurCityReport: React.FC<OurCityReportProps> = ({ folderName }) => {
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">Портал городских обращений</p>
          </div>
        </div>
      ...
      {/* Info */}
      <div className="flex items-start gap-3 px-4 py-3 bg-violet-50 dark:bg-violet-950/30 rounded-2xl border border-violet-100 dark:border-violet-900/50">
        <div className="w-7 h-7 bg-violet-100 dark:bg-violet-900/60 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5">
      ...
      {/* PDF */}
      <PDFViewer url={pdfUrl} title="Наш Город" />
    </div>
  );
};
