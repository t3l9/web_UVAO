...
import { ArrowLeft, FileSpreadsheet, Clock, Info, Monitor } from 'lucide-react';
import PDFViewer from './PDFViewer';
import { useReportFiles } from '../hooks/useReportFiles';

interface MayorMonitorReportProps {
  folderName: string;
}

const MayorMonitorReport: React.FC<MayorMonitorReportProps> = ({ folderName }) => {
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">Контрольные поручения мэра</p>
          </div>
        </div>
      ...
      {/* Info */}
      <div className="flex items-start gap-3 px-4 py-3 bg-blue-50 dark:bg-blue-950/30 rounded-2xl border border-blue-100 dark:border-blue-900/50">
        ...
        <p className="text-sm text-blue-700 dark:text-blue-300 leading-relaxed">
          Отчёт обновляется автоматически каждый час с 7:50 до 21:50.
        </p>
      </div>

      {/* PDF */}
      <PDFViewer url={pdfUrl} title="Монитор Мэра" />
    </div>
  );
};

export default MayorMonitorReport;
